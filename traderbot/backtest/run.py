...
def main():
    ...
    parser.add_argument("--strategy", default=None, help="Override strategy for selected tickers")
    ...
    # Переопределить стратегию для всех выбранных тикеров
    if args.strategy:
        from traderbot.strategies.registry import STRATEGY_REGISTRY
        if args.strategy not in STRATEGY_REGISTRY:
            logger.error("Unknown strategy: %s. Available: %s", args.strategy, ", ".join(STRATEGY_REGISTRY))
            sys.exit(1)
        for tc in tickers.values():
            tc.strategy = args.strategy
