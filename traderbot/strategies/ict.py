...
class ICTStrategy(BaseStrategy):
    """
    ROSN ICT — sweep 15 свечей 1H, EMA-тренд, RR 1:2.
    Изменения vs оригинал:
      - EMA20/50 на 1H — торгуем только по тренду
      - body_ratio 0.35→0.50, atr_ratio 0.70→0.85
      - ограничение displacement: 6 баров 30m после свипа
      - стоп за фитилём свиповой свечи
    """
    ...
    def find_setup(self, candles: dict[str, pd.DataFrame]) -> Setup | None:
        ...
        if len(df_1h) < max(MIN_CANDLES, EMA_SLOW + SWEEP_LOOKBACK + 2):
            return None
