...
class NVTKProStrategyExp(BaseStrategy):
    """
    NVTK PRO v3 EXP — экспериментальная версия.
    Отличие от основной: чистый RR target без обрезки до структуры.
    """
    ...
    def _build_smart_target(
        ...
        # EXP: чистый RR target без обрезки до структурного уровня
        return self._build_rr_target(entry, stop, direction, rr)
