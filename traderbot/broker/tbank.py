...
class TBankBroker:
    ...
    def place_market_order(
        ...
        """Разместить рыночный ордер. Вернуть order_id."""
        with self._client() as client:
            response = client.orders.post_order(
                figi=figi,
                quantity=qty,
                price=_float_to_quotation(0.0),
                direction=direction,
                account_id=account_id,
                order_type=OrderType.ORDER_TYPE_MARKET,
                order_id=str(uuid.uuid4()),
            )
        logger.info("[BROKER] Market order placed: %s, figi=%s, qty=%d",
                     response.order_id, figi, qty)
        return response.order_id
    ...
    def get_positions(self, account_id: str) -> list:
        """Получить список бумаг на счёте (для сверки с state бота)."""
        with self._client() as client:
            return list(client.operations.get_positions(
                account_id=account_id
            ).securities)
